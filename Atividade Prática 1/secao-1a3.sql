-- Parte 1 – Recuperando Dados (Seção 1) - utilizar comandos SELECT para exibir dados completos ou parciais de uma tabela.
...
-- Parte 2 – Filtros, Projeções e Concatenação (Seção 2) - trabalhar com cláusula WHERE, operadores BETWEEN, LIKE, IN, operadores de concatenação (||) e alias.
...
SELECT cidade FROM cliente WHERE cidade IN ('Niterói', 'Volta Redonda', 'Itaboraí');
...
-- Parte 3 – Ordenações, Operadores Lógicos e Funções (Seção 3) - utilizar ORDER BY, operadores lógicos (AND, OR, NOT) e funções de linha única (ROUND, UPPER, etc.).

-- 9. Exiba os dados de todas as contas com saldo superior a R$ 9.000, ordenados do maior para o menor saldo.
SELECT * FROM conta WHERE saldo > 9300 ORDER BY saldo DESC;
...
SELECT UPPER(cliente_nome) AS nome_maisculo FROM cliente;

-- 13. Exiba os nomes dos clientes que não são das cidades de Teresópolis nem Campos dos Goytacazes.
SELECT cliente_nome FROM cliente WHERE cidade NOT IN ('Teresópolis', 'Campo dos Goytacazes');